...
const main = () =>
  new Promise((resolve, reject) => {
    ...
    preparationPromise.then(() => {
      ...
      const audioStreamPromises = Promise.map(
        audioStreams,
        audioStream => {
      ...
      const videoStreamPromises = Promise.map(
      ...
      Promise.all([audioStreamPromises, videoStreamPromises])
        .then(([audioStreams, videoStrams]) => {
...
const addAudioStreamToEncoding = (output, aacCodecConfiguration, inputStreams, encoding, language) => {
  ...
  return new Promise((resolve, reject) => {
    addStream(encoding, stream, output, aacCodecConfiguration, language)
...
const addVideoStreamToEncoding = (input, output, videoCodecConfig, encoding) => {
  ...
  return new Promise((resolve, reject) => {
    addStream(encoding, stream, output, videoCodecConfig)
      .then(([addedStream, addedMuxing]) => {
              ...
              ([createdAudioAdaptationSet, createdVideoAdaptationSet]) => {
                Promise.map(audioMuxingsWithPath, (muxingWithPath, index) => {
                  ...
                  return bitmovin.encoding.manifests
                    .dash(createdManifest.id)
                    .periods(createdPeriod.id)
                    .adaptationSets(createdAudioAdaptationSet[index].id)
