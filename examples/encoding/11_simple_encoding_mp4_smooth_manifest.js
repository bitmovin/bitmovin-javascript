  ...
  return Promise.all(preparationPromises).then(results => {
    ...
    return Promise.all([audioPromises, videoPromises])
      .then(results => {
        ...
        const smoothManifestPromise = createSmoothManifest(output, encoding, audioMuxingsWithId, videoMuxingsWithId);
...
const addStreamToEncoding = (input, output, streamConfig, index, codecConfiguration, encoding) => {
  ...
  return new Promise((resolve, reject) => {
    addStream(encoding, stream, output, codecConfiguration, index, streamConfig.language)
...
const createSmoothManifest = (output, encoding, audioMuxingsWithPath, videoMuxingsWithPath) => {
  return new Promise((resolve, reject) => {
    createSmoothManifestResource(output)
...
const createSmoothManifestResource = output => {
  ...
  return new Promise((resolve, reject) => {
    bitmovin.encoding.manifests.smooth
      .create(manifest)
      .then(createdManifest => {
