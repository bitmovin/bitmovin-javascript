...
const OUTPUT_BASE_PATH = '/output/on-demand-javascript-' + new Date().toISOString();
...
const createMp4Muxings = (encoding, streamsWithFilenames, output) => {
  return streamsWithFilenames.map(stream => {
    const muxing = {
      ...
      outputs: [
        {
          outputId: output.id,
          outputPath: urljoin(OUTPUT_BASE_PATH, stream.outputPath),
          acl: [
            {
...
const addDashMp4RepresentationToAdaptationSet = (manifest, period, adaptationSet, encoding, muxing, profile) => {
  ...
  return bitmovin.encoding.manifests
    ...
    .representations.mp4.add(representation);
};

const createManifest = (output, videoManifestRepresentations, audioManifestRepresentations) =>
  new Promise(resolve => {
      ...
      return addPeriodToManifest(createdManifest).then(createdPeriod => {
        const videoRepresentationPromises = addVideoAdaptationSetToPeriod(createdManifest, createdPeriod).then(
          createdVideoAdaptationSet => {
            return Promise.map(
              videoManifestRepresentations,
              videoRepresentation => {
                return addDashMp4RepresentationToAdaptationSet(
                  createdManifest,
                  createdPeriod,
                  createdVideoAdaptationSet,
        ...
        Promise.all(streamPromises).then(([createdVideoStreams, createdAudioStreams]) => {
          ...
          Promise.all(mp4MuxingPromises).then(([createdVideoMuxings, createdAudioMuxings]) => {
            ...
            startEncodingAndWaitForItToBeFinished(createdEncoding).then(() => {
              const videoManifestRepresentations = createdVideoMuxings.map((videoMuxing, index) => {
                return {
                  muxing: videoMuxing,
              ...
              const audioManifestRepresentations = createdAudioMuxings.map((audioMuxing, index) => {
                return {
                  muxing: audioMuxing,
                  encoding: createdEncoding,
              ...
              createManifest(createdOutput, videoManifestRepresentations, audioManifestRepresentations).then(
                createdManifest => {
