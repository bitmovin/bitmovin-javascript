...
const main = () => new Promise((resolve, reject) => {
  ...
  const preparationPromises = [
    createHttpInputPromise, createS3OutputPromise, createH2641080pPromise,
    createH264720pPromise, createH264480pPromise, createH264360pPromise,
  ...
  preparationPromise.then(() => {
    ...
    const videoCodecConfigurations = [
      h264CodecConfiguration1080p, h264CodecConfiguration720p, h264CodecConfiguration480p,
      h264CodecConfiguration360p, h264CodecConfiguration240p
    ];

    addAudioStreamToEncoding(input, output, aacCodecConfiguration, encoding).then(audioStream => {

      const videoStreamMuxingsPromises = Promise.map(videoCodecConfigurations, (codecConfiguration) => {
...
const addAudioStreamToEncoding = (input, output, audioCodecConfiguration, encoding) => {
  ...
  return new Promise((resolve, reject) => {
    addStream(encoding, stream).then((addedStream) => {
...
const addVideoStreamToEncoding = (input, output, videoCodecConfig, audioStream, encoding) => {
  ...
  return new Promise((resolve, reject) => {
    addVideoStreamAndCreateMuxingWithVideoAndAudioStream(encoding, videoStream, output, videoCodecConfig.bitrate, audioStream).then((addedMuxing) => {
