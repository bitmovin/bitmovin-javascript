...
describe('[DRMs]', () => {
  ...
  const createFmp4Muxing = (encodingId, outputId) => {
    let promise = Promise.resolve(createStream(encodingId))
      .then(createdStream => {
        let fmp4Muxing = {
          name: 'Awesome FMP4',
  ...
  it('should add a playReady drm to fMP4 muxing', done => {
    ...
    Promise.all([encodingPromise, outputPromise])
      ...
      .then(createdMuxing => {
        ...
        return encodingsClient(createdEncoding.id)
          .muxings.fmp4(createdMuxing.id)
          .drms.playReady.add(playReadyDrm);
      })
      .then(createdDrm => {
        assert(createdDrm.id !== null && createdDrm.id !== undefined && createdDrm !== '');
  ...
  it('should add a primeTime drm to fMP4 muxing', done => {
    ...
    Promise.all([encodingPromise, outputPromise])
      ...
      .then(createdMuxing => {
        ...
        return encodingsClient(createdEncoding.id)
          .muxings.fmp4(createdMuxing.id)
          .drms.primeTime.add(primeTimeDrm);
      })
      .then(createdDrm => {
        assert(createdDrm.id !== null && createdDrm.id !== undefined && createdDrm !== '');
