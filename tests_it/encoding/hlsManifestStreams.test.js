...
describe('[HLS Manifest Media Tests]', () => {
  ...
  it('should add stream to a hls manifest', (done) => {
    ...
    manifestsClient(manifestResource.id).streams.add(sampleHlsStream).then((createdStream) => {
  ...
  it('should list streams of a hls manifest', (done) => {
    manifestsClient(manifestResource.id).streams.list(5).then((response) => {
  ...
  it('should return details of a stream of a hls manifest', (done) => {
    ...
    manifestsClient(manifestResource.id).streams.add(sampleHlsStream).then((createdStream) => {
      ...
      return manifestsClient(manifestResource.id).streams(createdStream.id).details();
    }).then((details) => {
  ...
  it('should delete a stream of a hls manifest', (done) => {
    ...
    manifestsClient(manifestResource.id).streams.add(sampleHlsStream).then((createdStream) => {
      compareHlsStreams(createdStream, sampleHlsStream);
      createdStreamResource = createdStream;
      return manifestsClient(manifestResource.id).streams(createdStream.id).delete();
    }).then((response) => {
