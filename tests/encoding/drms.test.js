        ...
        describe('drms', () => {
          ...
          const testDrm = (type) => {
            describe(type, () => {
              ...
              describe('list', () => {
                assertItCallsCorrectUrl('GET', `/v1/encoding/encodings/encoding-id/muxings/${muxingType}/muxing-id/drm/${type}`, client.list);
                assertItReturnsUnderlyingPromise(mockGet, client.list);
              });
              describe('add', () => {
                assertItCallsCorrectUrl('POST', `/v1/encoding/encodings/encoding-id/muxings/${muxingType}/muxing-id/drm/${type}`, client.add);
                assertItReturnsUnderlyingPromise(mockPost, client.add);
              });
              describe('drm', () => {
                describe('details', () => {
                  assertItCallsCorrectUrl('GET', `/v1/encoding/encodings/encoding-id/muxings/${muxingType}/muxing-id/drm/${type}/drm-id`, client('drm-id').details);
                  assertItReturnsUnderlyingPromise(mockGet, client('drm-id').details);
                });
                describe('customData', () => {
                  assertItCallsCorrectUrl('GET', `/v1/encoding/encodings/encoding-id/muxings/${muxingType}/muxing-id/drm/${type}/drm-id/customData`, client('drm-id').customData);
                  assertItReturnsUnderlyingPromise(mockGet, client('drm-id').customData);
                });
                describe('delete', () => {
                  assertItCallsCorrectUrl('DELETE', `/v1/encoding/encodings/encoding-id/muxings/${muxingType}/muxing-id/drm/${type}/drm-id`, client('drm-id').delete);
