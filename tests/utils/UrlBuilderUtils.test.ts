import {utils} from '../../bitmovin/utils/http';
describe('Tests creation of url out of given parameters', () => {
  ...
  describe('Test Url creation with given filtering', () => {
    it('Test with one filter', () => {
      ...
      expect(utils.buildGetParamString(filterParams)).toEqual('?type=VOD');
    });
    ...
    it('Test with two filters', () => {
      ...
      expect(utils.buildGetParamString(filterParams)).toEqual('?type=VOD&status=RUNNING');
    });
